...
use crate::communicator_objects::{
    ...
    UnityRlCapabilitiesProto, UnityRlInputProto,
...
impl GrpcServer {
    ...
    // Helper methods for SAC integration
    pub async fn send_input(&mut self, rl_input: crate::communicator_objects::UnityRlInputProto) -> Result<(), String> {
        let input = UnityInputProto {
            rl_input: Some(rl_input),
            rl_initialization_input: None,
        };
        ...
        self.trainer_to_unity_tx.send(msg)
            .map_err(|e| format!("Failed to send input: {}", e))
    }
    
    pub async fn receive_output(&mut self) -> Result<UnityOutputProto, String> {
        let response = self.unity_to_trainer_rx.recv().await
            .ok_or_else(|| "Unity disconnected".to_string())?;
